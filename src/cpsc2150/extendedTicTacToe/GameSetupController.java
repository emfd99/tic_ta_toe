...
public class GameSetupController {
    ...
    public void processButtonClick(int rows, int cols, int players, int numWin )
    {
        ...
        if(cols < IGameBoard.MIN_COLUMN || cols > max_size)
        {
            errorMsg += "Columns must be between " +  IGameBoard.MIN_COLUMN + " and " + max_size;
        }
        ...
        if(numWin < IGameBoard.MIN_NEEDED_TO_WIN)
        {
            errorMsg += "Number to win must be at least " + IGameBoard.MIN_NEEDED_TO_WIN;
        }
