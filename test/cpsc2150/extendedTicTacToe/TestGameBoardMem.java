...
public class TestGameBoardMem
{
     ...
     * file, instead of calling the constructor for GameBoardMem, we’ll call MakeAGameBoard. This
     * accomplishes the same thing, but it means the only reference to GameBoardMem will be
     * in the MakeAGameBoard method. This means when we want to test GameBoard, we just
     * need to change the constructor call inside of MakeAGameBoard, and no other piece of code
     * needs to change.
     * @return a memory-efficient implementation of IGameBoard
     * @post MakeAGameBoard(int row, int column, int numToWin) == new GameBoardMem(row, column, numToWin)
     */
    private IGameBoard MakeAGameBoard(int row, int column, int numToWin)
    {
        return new GameBoardMem(row, column, numToWin);
    }
